...
- [ ] **🔴 Finish the 2026-07-13 memoriams history scrub — residual exposure is still live** - the `git filter-repo` rewrite landed, but the follow-up action to have GitHub purge stale merged-PR refs was never done. Re-verified 2026-08-04: still outstanding. **Details, affected refs and the drafted support request are in the private repo** — `snh-private/memoriams_data/` (`history-scrub-2026-07-13.md` and `github-support-request.md`); they are deliberately not restated here, because naming them in a public file is itself a pointer. Owner: Rich. Also still unverified: whether the laptop's pre-scrub clone was re-cloned or hard-reset. (The local backup mirror is confirmed gone.)
