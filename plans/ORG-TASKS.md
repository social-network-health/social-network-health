...
- [ ] **Formally archive prt — harvest done, archive remaining** - the three-pass harvest (open issues, code, `ROADMAP.md`) completed 2026-08-16: sixteen issues filed on prm (prm#100–#115) plus one PNT design-note contribution — see [`2026-08-16-prt-harvest-crt-deltas.md`](2026-08-16-prt-harvest-crt-deltas.md) for the full landing map and the CRT-concept diff. (Earlier "prt#147 captured nowhere else" caveat was stale — the notification-protocol concepts live on in `research/protocols/`.) prt's README already carries the successor note; remaining step: archive the repo on GitHub. Substantive work stopped 2026-01-12
- [ ] **Point the shared `CLAUDE.md` block at the publication standard (block v8)** - `docs/publication-standards/identifiable-people.md` binds every repo, but nothing currently tells an agent working in prm or PNT that it exists. The shared block is the only channel that reaches every repo and every session, so it needs one line pointing at it — the same mechanism that carried the `snhb` rule in v7. Costs a full propagation round: edit the canonical copy, `just sync-conventions`, one PR per repo. Worth batching with the next block change rather than spending a round on one line
...
- [x] ~~**[org]** Write an org-wide publication standard for writing about identifiable people~~ (2026-08-05 — `docs/publication-standards/identifiable-people.md` v1.0, binding every document in every repo. Part A presentation rules apply always; Part B is a release gate whose *blocking vs deferred* setting turns on **reversibility**, not on how long a source has been public. Built from a survey of the WHO/Samaritans/Mindframe/#chatsafe reporting corpus, the AoIR/BPS/NESH internet-ethics corpus, and the CARE/ICMJE/COPE case-report corpus; the four research briefs are staged privately. Part C records, deliberately, the places where no standard exists — quoting a memoriam is ungoverned field-wide, which is what our own 2026-07-08 open question 2 had already spotted. Applied in the same PR to the threat catalog, which had drifted to the opposite posture from `memoriams.md`.)
